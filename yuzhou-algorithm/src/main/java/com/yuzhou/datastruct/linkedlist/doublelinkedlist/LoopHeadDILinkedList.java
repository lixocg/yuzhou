...
import com.yuzhou.datastruct.linkedlist.ILinkedList;
...
public class LoopHeadDILinkedList<T> implements ILinkedList<T> {
    ...
    protected Node<T> head; //不带数据的头结点
//   protected DNode<T> tail; //指向尾部的指针
    ...
    public LoopHeadDILinkedList(T[] array) {
        ...
        if (array != null && array.length > 0) {
            ...
            while (i < array.length) {
                p.next = new Node<>(array[i++], p, head);
                head.prev = p.next;
                p = p.next;
            }
        }
    ...
    public boolean add(int index, T data) {
        ...
        Node<T> q = new Node<>(data, p, p.next);
        p.next = q;
        p.next.prev = q;
    ...
    public boolean add(T data) {
        ...
        Node<T> p = new Node<>(data, head.prev, head);
        //更新tail后继指针的指向
