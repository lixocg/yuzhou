package com.yuzhou.datastruct.tree.AVLtree;
...
public class AVLTree<T extends Comparable<T>> implements Tree<T> {
    ...
    public int size(TNode<T> subtree) {
        if (subtree == null) {
            return 0;
        } else {
     ...
     * @return
     */
    public String preOrder(TNode<T> subtree) {
     ...
     * @return
     */
    private String inOrder(TNode<T> subtree) {
        StringBuilder sb = new StringBuilder();
        if (subtree != null) {
            //访问左子树
     ...
     * @return
     */
    private String postOrder(TNode<T> subtree) {
        StringBuilder sb = new StringBuilder();
        if (subtree != null) {
            //访问左子树
    ...
    public boolean contain(T data, TNode<T> subtree) {
     ...
     * @return
     */
    private TNode<T> singleRotateLeft(TNode<T> x) {
        //把w结点旋转为根结点
        TNode<T> w = x.left;
        //同时w的右子树变为x的左子树
        x.left = w.right;
     ...
     * @return
     */
    private TNode<T> singleRotateRight(TNode<T> w) {
     ...
     * @return
     */
    private TNode<T> doubleRotateWithLeft(TNode<T> x) {
        //w先进行RR旋转
     ...
     * @return
     */
    private TNode<T> doubleRotateWithRight(TNode<T> w) {
        //先进行LL旋转
    ...
    private void printTree(TNode<T> t) {
        if (t != null) {
            printTree(t.left);
