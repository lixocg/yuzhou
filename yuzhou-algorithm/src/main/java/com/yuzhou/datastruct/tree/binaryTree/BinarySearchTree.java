...
import com.yuzhou.datastruct.tree.Tree;
...
public class BinarySearchTree<T extends Comparable<T>> implements Tree<T> {
     ...
     */
    public TNode<T> createBinarySearchTreeByPreIn(T[] preList, T[] inList, int preStart, int preEnd, int inStart, int inEnd) {
        //preList[preStart]必须根结点数据,创建根结点root
        TNode<T> p = new TNode<>(preList[preStart]);
        //如果没有其他元素,就说明结点已构建完成
     ...
     * @return 根结点
     */
    public TNode<T> createBinarySearchTreeByPostIn(T[] postList, T[] inList, int postStart, int postEnd, int inStart, int inEnd) {

        //构建根结点
        TNode<T> p = new TNode<>(postList[postEnd]);
     ...
     * @return
     */
    private int size(TNode<T> subtree) {
        if (subtree == null)
            return 0;
        else {
     ...
     * @return
     */
    private int height(TNode<T> subtree) {
        if (subtree == null) {
            return 0;
        } else {
     ...
     * @return
     */
    private String preOrder(TNode<T> subtree) {
     ...
     * @return
     */
    public String inOrder(TNode<T> subtree) {
        StringBuilder sb = new StringBuilder();
        if (subtree != null) {//递归结束条件
            //先遍历左子树
     ...
     * @return
     */
    public String postOrder(TNode<T> subtree) {
        StringBuilder sb = new StringBuilder();
        if (subtree != null) {//递归结束条件
            //先遍历左子树
    ...
    public String postOrderTraverse() {
        ...
        TNode<T> currentNode = this.root;
        TNode<T> prev = this.root;
     ...
     * @return
     */
    private TNode<T> remove(T data, TNode<T> p) {
        //没有找到要删除的元素,递归结束
        if (p == null) {
            return p;
        }
    ...
    public boolean removeUnrecure(T data) {
        ...
        //删除带有两个孩子结点的结点
            ...
            TNode<T> successor = findSuccessor(current);
     ...
     * @return
     */
    public TNode<T> findSuccessor(TNode<T> delNode) {
        TNode<T> successor = delNode;
        TNode<T> successorParent = delNode;
        TNode<T> current = delNode.right;
     ...
     * @return
     */
    private TNode<T> findMin(TNode<T> p) {
     ...
     * @return
     */
    private TNode<T> findMax(TNode<T> p) {
        if (p == null)//结束条件
            return null;
     ...
     * @return
     */
    private TNode<T> findNode(T data, TNode<T> p) {
    ...
    private boolean contains(T data, TNode<T> p) {
     ...
     */
    public void print() {
        LinkedList<TNode<T>> tree = getCompleteBinaryTree();
        //获取树的深度
        int depth = height();
        Iterator<TNode<T>> iterator = tree.iterator();
     ...
     */
    private LinkedList<TNode<T>> getCompleteBinaryTree() {
        Queue<TNode<T>> queue = new LinkedList<>();
        LinkedList<TNode<T>> tree = new LinkedList<>(); // 把树补充成完全二叉树，放在这个链表中
        queue.add(root);
        TNode<T> empty = null;
        ...
        while (queue.size() > 0 && nodeCount > 0) {
            ...
            if (node != null) {
                nodeCount--;
                tree.add(node);
                TNode<T> left = node.left;
                TNode<T> right = node.right;
                if (left == null) {
                    queue.add(empty);
                } else {
//                    queue.add(linkFlag);
