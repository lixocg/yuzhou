...
import com.yuzhou.qiyukf.aes.WXBizMsgCrypt;
import com.yuzhou.qiyukf.controller.wxservice.WXAuthService;
import com.yuzhou.qiyukf.controller.wxservice.WXUserService;
import com.yuzhou.qiyukf.controller.wxservice.WxMessageService;
import com.yuzhou.qiyukf.controller.wxutil.SHA1;
import com.yuzhou.qiyukf.controller.wxutil.WXOpenException;
import com.yuzhou.qiyukf.session.constant.OpenApiTags;
import com.yuzhou.qiyukf.session.model.ApplyStaffInfo;
import com.yuzhou.qiyukf.session.model.ApplyStaffResult;
import com.yuzhou.qiyukf.session.model.CommonResult;
import com.yuzhou.qiyukf.session.model.Session;
import com.yuzhou.qiyukf.session.util.StringUtil;
...
public class OfficialAccountController {
    ...
    public String onUnicornMsg(@RequestParam(value = "time") Long time,
        ...
        try {
            ...
            log.info("receive qiyu: " + eventType + " content: " + content);
            return qiyuMessageReceiver.onReceive(time, checksum, eventType, content);
        } catch (Throwable e) {
            log.warn("onUnicornMsg error: " + e);
        }
        return "";
    }
     ...
     * 企微客户
     */
    @RequestMapping(value = "/recv_wxkf")
    @ResponseBody
    public String onWxkfMessage(
            ...
            InputStream is) {
        try {
            String corpId = "ww7a485d204fba13bb";
            String aesKey = "7Qy4jtOA0yyCqkJogOvNICe49tKy69AOuiNlR0o2jWV";
            WXBizMsgCrypt wxBizMsgCrypt = new WXBizMsgCrypt(Constants.WX_TOKEN,aesKey,corpId);
            if (TextUtils.isEmpty(echoStr)) {
                ...
                // 解析XML， 分离消息来源，内容
                String decryptMsg = wxBizMsgCrypt.DecryptMsg(signature, timestamp, nonce, msg);
    ...
    public String onWxMessage(
        ...
        try {
            if (TextUtils.isEmpty(echoStr)) {
                ...
                log.info("verify url: " + sha1 + " - " + signature + " - " + echoStr);
...
//                });
            ...
            log.error("parse WXMessage error, " + e);
    ...
    private void handleNormalWxMsg(String msgType, Element root) {
        ...
        if (!shouldIntercept(fromUser, msgType, root)) {
            try {
                ...
                log.info("forward message result: " + result);
            } catch (Exception e) {
                log.info("forward message error: " + e);
            }
        }
    }
        ...
        if ("text".equals(msgType)) {
            ...
            if (("RG".equalsIgnoreCase(content) || "人工".equals(content)) && !sessionManager.isInSession(fromUser)) {
                ...
                try {
                    ...
                    log.info("ApplyStaffResult: " + result.getCode() + ", message:" + result.getMessage() + ", count: " + result.getCount());
